  ...
  const playCurrent = () => {
    ...
    videoElement.play().catch(err => console.error("Error al reproducir:", err))
  }
  ...
  // Autoactualización cada 5 minutos solo si el video no está sonando
  setInterval(async () => {
    if (videoElement.paused || videoElement.ended) {
      ...
      if (!error && data && data.length > 0) {
        playlist = data.map(v => v.url)
        currentIndex = 0
        console.log("🎬 Playlist actualizada automáticamente:", playlist)
      }
    }
  }, 5 * 60 * 1000)
